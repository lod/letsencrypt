...
class RedirectTest(util.ApacheTest):
    """Test two standard well-configured HTTP vhosts."""

    def setUp(self):  # pylint: disable=arguments-differ
        tbasedir = "debian_apache_2_4/redirect_vhost"
        super(RedirectTest, self).setUp(test_dir=tbasedir, config_root=tbasedir+"/apache2", vhost_root=tbasedir+"/apache2/sites-available")

        self.config = util.get_apache_configurator(
            self.config_path, self.vhost_path, self.config_dir, self.work_dir)
        print "TL21: %d" % len(self.config.vhosts)
        self.config = self.mock_deploy_cert(self.config)
        print "TL22: %d" % len(self.config.vhosts)
        self.vh_truth = util.get_vh_truth(
            self.temp_dir, "debian_apache_2_4/redirect_vhost")
        print "TL23: %d" % len(self.config.vhosts)
    ...
    def test_get_all_names_j(self, mock_getutility):
        print "TL31: %d" % len(self.config.vhosts)
        mock_getutility.notification = mock.MagicMock(return_value=True)
        print "TL32: %d" % len(self.config.vhosts)
        names = self.config.get_all_names()
        print "TL33: %d" % len(self.config.vhosts)
